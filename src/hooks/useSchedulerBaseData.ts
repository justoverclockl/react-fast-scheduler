...
>({
  ...
  resources,
  selectedDate
}: UseSchedulerBaseDataArgs<TAppointment, TResource, TResourceId>): SchedulerBaseData<TAppointment, TResourceId> {
  const dayStartAbs = React.useMemo(() => parseHHMM(dayStart), [dayStart]);
  const dayEndAbs = React.useMemo(() => parseHHMM(dayEnd), [dayEnd]);
  const dayMinutes = React.useMemo(() => Math.max(0, dayEndAbs - dayStartAbs), [dayEndAbs, dayStartAbs]);
  const gridHeight = React.useMemo(() => TOP_PAD + dayMinutes * PX_PER_MIN + BOTTOM_PAD, [dayMinutes]);
  const gridMinWidth = React.useMemo(() => GUTTER_W + Math.max(resources.length, 1) * RESOURCE_MIN_W, [resources.length]);
